...
import { stringOrStringArray } from '../../utils/comma-separated-list-param.js';
...
export const universalSearchUsersSchema = z.object({
  ...
  // Response control
  fields: stringOrStringArray.optional().describe('Specific user fields to return. Accepts a comma-separated string or an array.'),
  expand: stringOrStringArray.optional().describe('Expand user details (groups, applicationRoles). Accepts a comma-separated string or an array.')
});
...
// Tool registration
export const registerUniversalSearchUsersTool = (server: ToolRegistrar) => {
  server.tool(
