...
import { stringOrStringArray, toCommaString } from '../../utils/comma-separated-list-param.js';
...
export const getSprintIssuesSchema = z.object({
  ...
  fields: stringOrStringArray.optional().describe('Comma-separated list of fields to include (default: summary,status,assignee,reporter,priority). Accepts a comma-separated string or an array.'),
  expand: stringOrStringArray.optional().describe('Expand options (e.g., "names,schema,operations"). Accepts a comma-separated string or an array.')
});
...
async function getSprintIssuesImpl(params: GetSprintIssuesParams, context: any) {
  ...
  try {
    ...
    const fields = toCommaString(params.fields);
    if (fields) queryParams.append('fields', fields);
    const expand = toCommaString(params.expand);
...
export const registerGetSprintIssuesTool = (server: ToolRegistrar) => {
  server.tool(
    'getSprintIssues',
