...
export const getIssueCommentsSchema = z.object({
  ...
  expand: stringOrStringArray.optional().describe('Expand options for comments (e.g., "renderedBody,properties"). Accepts a comma-separated string or an array.')
});
...
async function getIssueCommentsImpl(params: GetIssueCommentsParams, context: any) {
  ...
  try {
    ...
    const comments = data.comments?.map((comment: any) => ({
      ...
      // Comment bodies are ADF documents; render to Markdown for the client.
...
export const registerGetIssueCommentsTool = (server: ToolRegistrar) => {
  server.tool(
    'getIssueComments',
